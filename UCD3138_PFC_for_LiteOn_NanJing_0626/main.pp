...
union STATUS_1
{
 ...
 union STATUS_0 status_0;
 union STATUS_1 status_1, status_1_hold;

 struct INTERRUPT_VARIABLES iv;

 int32 switching_frequency;

//stuff for UART communication
 Uint8 uart_rx_buf[(6)];
 Uint8 uart_text_in_buf[(6)];
 Uint8 uart_rx_data_rdy; //flag, received a new data packet
 Uint8 uart_rx_buf_ptr; //point to the buffer which will store the coming data
 int32 uart_rx_timer; //count IRQ, used to count the UART receive idle period
 int32 uart_tx_timeout; //used to count the timeout for message transmission
 int32 uart_tx_checksum; //checksum for transmit
 int32 primary_secondary_count;
...
//stuff for PMBus communication
 ...
 Uint8 ipm_or_zvs;
 Uint8 zvs_flag;//turn on/off ZVS
...
//stuff for IPM
 Uint32 emi_capacitance;
 Uint32 emi_resistance;
 Uint32 emi_discharge_resistance;
 Uint32 iin_rms;
...
union HK_P2S_FRAME
{
 ...
 union HK_P2S_FRAME bufferTX;
 union HK_S2P_FRAME bufferRX;

 unsigned char fw_revision[4];
 Uint32 pfc_voltage;

 Uint16 output_current;
 Uint16 standby_output_current;
 Uint16 vac_from_meter_ic;
 Uint8 uart_rx_buf_err[(6)];


 Uint16 PMBUS_L11_Vin;
 Uint16 PMBUS_L11_Iin;
 Uint16 PMBUS_L11_Pin;
 int mantissa;
 Uint16 uExponent;
 Uint16 uMantissa;

 Uint32 ac_frequency;
 Uint32 rampup_target;
 Uint32 rampdown_target;
 Uint32 pri_temp1;
 Uint32 ext_temp1;
...
// UART Autobaud Rate Detection
 Uint32 pulse_width;
 Uint32 baud_div_value;
 Uint8 T24SREG;
 Uint8 edge;
 Uint32 result;
 Uint32 ac_recover_counter;
 Uint32 xflag;
 Uint32 vbus_ok;
 Uint32 ac_input;
 Uint32 dc_input;
 Uint32 dc_ac_half_load_rampdown_voltage;
 Uint32 dc_ac_half_load_vbus_good_off;
 Uint32 vbus_mode;
 Uint32 pre_vbus_mode;

 Uint32 rampdown_notcomplete;
 Uint32 rampup_notcomplete;

 Uint32 brown_in_voltage;
 Uint32 brown_out_voltage;
 Uint32 vin_min_on_sq_avg;
 Uint32 vin_min_off_sq_avg;
 Uint32 vin_min_off;
struct MASS_DEBUG_STRUCTURE{//[Ken Zhang]add for mass variable debug
 ...
 struct MASS_DEBUG_STRUCTURE meDebug;
...
typedef struct _USER_DATA_PAGE
{
 ...
 USER_DATA_PAGE UserData;
 Uint32 standard_int_freq;
 Uint32 e_meter_version;
...
typedef struct 	//for current loop coefficients
{
 ...
 PFC_CONFIG_STRUCT pfc_config_in_ram;
...
{
	if (((rx_byte == '+') || (rx_byte == '='))
			&& (iv.vbus_target <= (((int32)((430<<12)/(527))) - 10))) {
		iv.vbus_target += 10;
...
inline void voltage_feed_forward(void)	//calculate Km/Vrms^2
{
	...
	{
		if (iv.vin_squared_average < ((Uint32)((((167.0) * (167.0))/((399) * (399))) * 32768))) //if VAC is below normal operating range
		{
			iv.vff_multiplier = ((Uint32)(0.5 * (167.0) * 1.414 * (5.11/815.11) * 0x7FFFFFFF)) / ((Uint32)((((167.0) * (167.0))/((399) * (399))) * 32768)); //Q30/Q15 = Q15 limit to minimum operating voltage to avoid overflow
		} else //here if vac is within range
		{
			if (abs(
					...
					{
				iv.vff_multiplier = ((Uint32)(0.5 * (167.0) * 1.414 * (5.11/815.11) * 0x7FFFFFFF)) / iv.vin_squared_average;//Q30/Q15 = Q15
			} else //here if the fast and slow values are close - use the slow value.
			{
				if (iv.vin_squared_slow_average
						< (((Uint32)((((167.0) * (167.0))/((399) * (399))) * 32768))
								<< (15))) {
					iv.vff_multiplier = ((Uint32)(0.5 * (167.0) * 1.414 * (5.11/815.11) * 0x7FFFFFFF)) / ((Uint32)((((167.0) * (167.0))/((399) * (399))) * 32768)); //Q30/Q15 = Q15 limit to minimum operating voltage to avoid overflow
				} else {
					iv.vff_multiplier = ((Uint32)(0.5 * (167.0) * 1.414 * (5.11/815.11) * 0x7FFFFFFF))
							/ (iv.vin_squared_slow_average
									>> (15)); //Q30/Q15 = Q15
				}
			}
...
{
	erase_one_section(56, sizeof(UserData.Calibration));//20140526--Kevin.Lai
...
//		MiscAnalogRegs.GLBIOVAL.bit.DPWM0A_IO_VALUE	= 0;
		...
		if (bufferRX.S2P.status.bit.Standby_mode != 1) {
			...
			if ((iv.vbus_avg >= ((int32)((375<<12)/(527))))
					&& (vbus_mode == 1)) //if Vbus voltage is good
					{
...
//						}
			...
			else if ((iv.vbus_avg >= ((int32)((345<<12)/(527))))
					&& ((vbus_mode == 3)
							|| (vbus_mode == 2)))
					{
...
//			else if(iv.vbus_target == VBUS_RAMP_VOLTAGE_3)//351V
			...
			if (iv.vbus_raw <= ((int32)((320<<12)/(527))))
			{
				MiscAnalogRegs.GLBIOVAL.bit.ALERT_IO_VALUE = 1;	//output high for Vbus not ok
...
//			else if(iv.vbus_target == iv.vbus_setpoint)//410V
			...
			if (iv.vbus_raw <= ((int32)((345<<12)/(527))))
			{
				MiscAnalogRegs.GLBIOVAL.bit.ALERT_IO_VALUE = 1;	//output high for Vbus not ok
//					MiscAnalogRegs.GLBIOVAL.bit.DPWM0A_IO_VALUE	= 1;
				vbus_ok = 0;
