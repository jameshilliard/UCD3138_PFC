...
//    	}
		if((iv.vbus_setpoint<(iv.vbus_avg-10*4095/(527)) && iv.pis.i>0) || ((iv.vbus_setpoint-10*4095/(527))>iv.vbus_avg && iv.pis.i<0)){
			iv.pis.i=0;
		}
	}
...
inline void calculate_current_target_shunt(void)
{
	...
	if((iv.vbus_setpoint-155)<iv.vbus_raw){//[Ken Zhang]Vbus>setpoint-20=390
		...
		if(iv.i_target_sensed > (0x170)){ //saturate current target at maximum current
			iv.i_target_sensed = (0x170);
		}
	}
	else{
...
{
	if(iv.adc_raw[(1)] > iv.adc_raw[(2)] )//this is the cycle for line
	{
		iv.vin_raw = iv.adc_raw[(1)] - iv.adc_raw[(2)];
		iv.positive = 1;
	}
	else //cycle for neutral
	{
		iv.vin_raw = iv.adc_raw[(2)] - iv.adc_raw[(1)];	
		iv.positive = 0;
	}
...
//			MiscAnalogRegs.GLBIOVAL.bit.DPWM0A_IO_VALUE	= 1;
	...
	iv.vin_squared_slow_average = iv.vin_squared_slow_average + (iv.vin_squared >> 9)-
									(iv.vin_squared_slow_average >> (15)); 
									//Q15 + shift of 15 gives Q30
	}
	if(iv.vin_average > (204))
	{
	...
	else
	{
		...
		if((iv.ac_drop_count > (10))&& (iv.ac_drop_continue == 0))
		{
			if((iv.supply_state == STATE_PFC_ON)&&(!iv.ac_drop))
			{
	...
	if(iv.vin_average > (204))//(iv.vin_squared_for_ac_drop > AC_UNDROPPED_THRESHOLD) //if above ac not dropped threshold
	{
...
//	if((iv.vin_squared_for_ac_drop > AC_UNDROPPED_THRESHOLD)) //if above ac not dropped threshold
	if((iv.vin_squared_for_ac_drop > ((Uint32)(((170.0)*(170.0)/((399) * (399))) * 32768)))&&bufferTX.P2S.status.bit.INPUT_OK==0) //[Ken Zhang] and for timer overflow issue
	{ 
...
//					if(pri_temp1<t_pri_min) t_pri_min=pri_temp1;//[Ken Zhang] add for test Tpri bounce
		...
		if ((iv.ac_drop == 0) && (iv.vbus_raw > ((int32)((430<<12)/(527)))))
...
{
	if(uart_tx_timeout <= (3000)) //1000ms, count time since the last packet was sent
	{
		uart_tx_timeout++;
	}
	...
	if(uart_rx_data_rdy == 0) //receive data, one byte a time
	{
		if(Uart0Regs.UARTRXST.bit.RX_RDY == 1)
		{
			...
			uart_rx_timer = 0; 
			if(uart_rx_buf_ptr == (6) ) //received all bytes
			{
...
//	MiscAnalogRegs.GLBIOVAL.bit.DPWM0A_IO_VALUE	= 1;
	...
	if(ac_input==1)
	{
		brown_in_voltage = ((Uint32)(((175.0) * 1.414 * 4096) / (399)));
		vin_min_on_sq_avg = ((Uint32)((((175.0) * (175.0))/((399) * (399))) * 32768));
		vin_min_off_sq_avg = ((Uint32)((((167.0) * (167.0))/((399) * (399))) * 32768));
		vin_min_off = (167.0);
	}
	else if(dc_input==1)
	{
		brown_in_voltage = ((Uint32)(((173.0) * 4096) / (399)));
		vin_min_on_sq_avg = ((Uint32)((((173.0) * (173.0))/((399) * (399))) * 32768));
		vin_min_off_sq_avg = ((Uint32)((((143.0) * (143.0))/((399) * (399))) * 32768));
		vin_min_off = (143.0);
	}
	else
	{
	...
	if((iv.vin_peak > brown_in_voltage)&& (iv.ac_drop == 0))//if Vac above 87 volts
	{
		if(ac_input==1)
		{
			dc_ac_half_load_rampdown_voltage = ((Uint32)((((Uint32)(350))*4095)/(527)));
			dc_ac_half_load_vbus_good_off = ((int32)((320<<12)/(527)));
		}
		else if(dc_input ==1)
		{
			dc_ac_half_load_rampdown_voltage = ((Uint32)((((Uint32)(341))*4095)/(527)));
			dc_ac_half_load_vbus_good_off = ((int32)((310<<12)/(527)));
		}
		else
		{
			dc_ac_half_load_rampdown_voltage = ((Uint32)((((Uint32)(350))*4095)/(527)));
...
/*			if((bufferRX.S2P.status.bit.PFC_DIS==1)&&(bufferRX.S2P.status.bit.Standby_mode==1)&&(iv.vin_squared_average > VAC_STANDBY_PFC_DIS))//||1)//[Ken Zhang] disable PFC
	...
	else
	{
		MiscAnalogRegs.GLBIOVAL.bit.CONTROL_IO_VALUE = 1;//active high for AC NON OK
		bufferTX.P2S.status.bit.INPUT_OK = 0;
		if(iv.vin_squared_average <= ((Uint32)((((55.0)*(55.0))/((399) * (399))) * 32768)))
		{
...
inline void relay_bounce_state_handler(void)
{
	...
	if((iv.interrupt_counter_1 >= 100)&&(iv.vbus_avg > ((int32)((180<<12)/(527)))))//10ms
	{
...
//	MiscAnalogRegs.GLBIOVAL.bit.DPWM0A_IO_VALUE	= 0;
	...
	else if(pre_vbus_mode==2)
	{
		vbus_mode=2;
	}
	else if(pre_vbus_mode==3)
	{
		vbus_mode=3;
	}
	else if(pre_vbus_mode==4)
	{
		vbus_mode=4;
	}
	else
...
//	if(iv.vin_average > VAC_MIN_OFF)
	{
		if(iv.vin_squared_average > ((Uint32)(((220.0*220.0)/((399) * (399))) * 32768)))
		{
...
inline void ramp_down_state_handler(void)
{
	...
	if((vbus_mode==4)&&(bufferRX.S2P.status.bit.Standby_mode==0))
	{
		iv.interrupt_counter_1 = 0;
		rampdown_notcomplete = 0;
		iv.supply_state = STATE_PFC_ON;
...
{		
	if(iv.vbus_avg < ((int32)((400<<12)/(527))))
	{
			iv.pis.i = 0;
			xflag = 0;
