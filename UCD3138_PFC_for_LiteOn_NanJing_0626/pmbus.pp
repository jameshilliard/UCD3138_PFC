...
const Uint8 vbus_full_range_text[] = "527";
...
const Uint8 setup_id[] = "VERSION1|PFC001";
const Uint8 cmd_pfc[]  = {0x00, 0x00, 0x60, 0x00, 0xC0, 0x00, 0x10, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x05, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x60, 0x3F, 0x00, 0x85, 0x04 };
const Uint8 cmd_dcdc[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xD0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
Uint8 pmbus_write_vout_ov_fault_limit(void)
{
	...
	FaultMuxRegs.ACOMPCTRL0.bit.ACOMP_B_THRESH = ((int32)((temp * 127) / (527)));
...
Uint8 pmbus_read_vout_ov_fault_limit(void)
{
	...
	temp = ((Uint32)FaultMuxRegs.ACOMPCTRL0.bit.ACOMP_B_THRESH * (527)) / 127;
...
{
	Uint8 byte;
	pmbus_number_of_bytes = sizeof("VERSION1|PFC001") + 1;

	if (pmbus_number_of_bytes <= 40)
	{
		for (byte = 0; byte < sizeof("VERSION1|PFC001"); byte++)
		{
			pmbus_buffer[byte+1] = (Uint8)setup_id[byte];
		} 
	}	   
	pmbus_buffer[0] = sizeof("VERSION1|PFC001");
	return 0;
}
...
{ 
	Uint8 byte;
	pmbus_number_of_bytes = sizeof("Debug 0") + 1;

	switch (gui_constant_pointer) 			// look at command byte from a write perspective
	{
		case 10: 
			pmbus_number_of_bytes = sizeof("Debug 0") + 1;
			for (byte = 0; byte < sizeof("Debug 0"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_0_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 0");
			break;
		case 11: 
			pmbus_number_of_bytes = sizeof("Debug 1") + 1;
			for (byte = 0; byte < sizeof("Debug 1"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_1_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 1");
			break;
		case 12: 
			pmbus_number_of_bytes = sizeof("Debug 2") + 1;
			for (byte = 0; byte < sizeof("Debug 2"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_2_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 2");
			break;
		case 13: 
			pmbus_number_of_bytes = sizeof("Debug 3") + 1;
			for (byte = 0; byte < sizeof("Debug 3"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_3_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 3");
			break;
		case 14: 
			pmbus_number_of_bytes = sizeof("Debug 4") + 1;
			for (byte = 0; byte < sizeof("Debug 4"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_4_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 4");
			break;
		case 15: 
			pmbus_number_of_bytes = sizeof("Debug 5") + 1;
			for (byte = 0; byte < sizeof("Debug 5"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_5_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 5");
			break;
		case 16: 
			pmbus_number_of_bytes = sizeof("Debug 6") + 1;
			for (byte = 0; byte < sizeof("Debug 6"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_6_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 6");
			break;
		case 17: 
			pmbus_number_of_bytes = sizeof("Debug 7") + 1;
			for (byte = 0; byte < sizeof("Debug 7"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)debug_7_text[byte];
			} 
			pmbus_buffer[0] = sizeof("Debug 7");
			break;
			
		case 110: 
			pmbus_number_of_bytes = sizeof("527") + 1;
			for (byte = 0; byte < sizeof("527"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)vbus_full_range_text[byte];
			} 
			pmbus_buffer[0] = sizeof("527");
			break;
		case 111: //send "yes" if TFA is enabled
			pmbus_number_of_bytes = sizeof("N/A") + 1;
			for (byte = 0; byte < sizeof("N/A"); byte++)
			{
				pmbus_buffer[byte+1] = (Uint8)na_text[byte];
			} 
			pmbus_buffer[0] = sizeof("N/A");
			break;
		case 112: 
			pmbus_number_of_bytes = 2;
...
int32 pmbus_write_message(void)
{
	...
	{
		case 0xD9: 			return pmbus_write_rom_mode();
		case 0x11: 	return pmbus_write_store_default_all();
		case 0x12: return pmbus_write_restore_default_all();
		case 0x21: 		return pmbus_write_vout_cmd();
		case 0xF7: return pmbus_write_pi_coefficients();
		case 0x40: return pmbus_write_vout_ov_fault_limit();
		case 0x33: 	return pmbus_write_frequency();
		case 0xDA: 		return pmbus_write_user_ram_00();
		case 0xF5: 	return pmbus_write_gui_constant();
        case 0xE2: 		return pmbus_write_parm_info();
		case 0xE3: 		return pmbus_write_parm_value();
		case 0xBD:		return pmbus_write_calibration_data();
		default:
			break;
	}
	return 0; 
...
int32 pmbus_read_message(void)
{
	...
	{
		case 0x20: 				return pmbus_read_one_byte_handler(0x0); //linear, 0 exponent
		case 0x8B: 				return pmbus_read_vout_handler();
		case 0x98: 			return pmbus_read_one_byte_handler(0x11); //version 1.1 for both
		case 0x99: 					return pmbus_mfr_id_handler(); 
		case 0x9A: 				return pmbus_mfr_model_handler(); 
		case 0x9B: 			return pmbus_mfr_revision_handler(); 
        case 0xE4: 	return pmbus_read_cmd_dcdc_paged();
        case 0xE5: 	return pmbus_read_cmd_dcdc_nonpaged();
        case 0xE6: 			return pmbus_read_cmd_pfc();		
		case 0xE7: 			return pmbus_read_setup_id();
		case 0xFD: 			return pmbus_read_device_id() ;
		case 0x33: 		return pmbus_read_two_byte_handler(switching_frequency);					
		case 0x95:			return pmbus_read_two_byte_handler(switching_frequency);
		case 0xF0: 		return pmbus_read_debug_buffer();
		case 0x21:        	return pmbus_read_vout_cmd();
		case 0xF5: 		return pmbus_read_gui_constant();
		case 0xF7: 	return pmbus_read_pi_coefficients();
		case 0xDA: 			return pmbus_read_user_ram_00();
		case 0x40: 	return pmbus_read_vout_ov_fault_limit();
		case 0xE2: 			return pmbus_read_parm_info();
		case 0xE3: 			return pmbus_read_parm_value();
		case 0x89:				return pmbus_read_iin();
		case 0x97:				return pmbus_read_pin();
		case 0xBE: 			return pmbus_read_calibration_info();
		default:
