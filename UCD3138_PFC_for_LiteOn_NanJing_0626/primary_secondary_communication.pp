...
//		string_out_0("uart_rx_data_rdy");
		int32 checksum, i;
		checksum = 0;
		for(i=0; i<((6)-1); i++)  //calculate checksum
		{
			checksum += uart_text_in_buf[i];
		}
		checksum = checksum &0xff;
		checksum = 0x100 - checksum;
		if((uart_text_in_buf[0]==0xAA)&&(checksum == uart_text_in_buf[(6)-1])) //process received data if checksum is correct and header is correct
		{
...
//		pfc_voltage = iv.vbus_avg * VBUS_FULL_RANGE /4096;
		pfc_voltage = (iv.vbus_avg * (527))>>12;
